                       ...
                       DataType    feature[NUM_FEATURES])
{
  ...
  DOT: for (int i = 0; i < NUM_FEATURES / UNROLL_FACTOR; i++)
  {
    #pragma HLS PIPELINE II=1
    DOT_INNER: for(int j = 0; j < UNROLL_FACTOR; j++) 
    {
      FeatureType term = param[i*UNROLL_FACTOR+j] * feature[i*UNROLL_FACTOR+j];
      result += term;
    }
  }
  return result;
...
{
  #pragma HLS INLINE
  GRAD: for (int i = 0; i < NUM_FEATURES / UNROLL_FACTOR; i++)
  {
    #pragma HLS PIPELINE II=1
    GRAD_INNER: for (int j = 0; j < UNROLL_FACTOR; j++)
      grad[i*UNROLL_FACTOR+j] = (scale * feature[i*UNROLL_FACTOR+j]);
  }
}
    ...
    FeatureType scale)
{
  ...
  UPDATE: for (int i = 0; i < NUM_FEATURES / UNROLL_FACTOR; i++)
  {
    #pragma HLS PIPELINE II=1
    UPDATE_INNER: for (int j = 0; j < UNROLL_FACTOR; j++)
      param[i*UNROLL_FACTOR+j] += scale * grad[i*UNROLL_FACTOR+j];
  }
}
