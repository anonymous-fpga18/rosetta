...
LabelType knn_vote( int knn_set[UNROLL_FACTOR * K_CONST] ) 
{
  // local buffers
  ...
  LANES: for (int i = 0; i < UNROLL_FACTOR; i ++ )
  {
    INSERTION_SORT_OUTER: for (int j = 0; j < K_CONST; j ++ )
    {
      ...
      INSERT: for (int r = K_CONST ;r > 0; r -- )
      {
        ...
        else if (r-1 == pos)
        {
          ...
          label_list[r-1] = i / (UNROLL_FACTOR / 10);
        }
      }
    }
  }
...
__kernel void DigitRec(__global WholeDigitType * global_training_set, __global WholeDigitType * global_test_set, __global LabelType* global_results) 
{
  ...
  int knn_set[UNROLL_FACTOR * K_CONST] __attribute__((xcl_array_partition(complete, 1)));

  // local copies of arrays
  __local WholeDigitType training_set [NUM_TRAINING] __attribute__((xcl_array_partition(block, UNROLL_FACTOR, 1)));
  __local WholeDigitType test_set     [NUM_TESTING];
  __local LabelType results           [NUM_TESTING];

  data_copy[0] = async_work_group_copy(training_set, global_training_set, NUM_TRAINING, 0);
  data_copy[1] = async_work_group_copy(test_set,     global_test_set,     NUM_TESTING,  0);

  wait_group_events(2, data_copy);

  // loop through test set
  TEST_LOOP: for (int t = 0; t < NUM_TESTING; ++t) 
  {
    // fetch one instance
    ...
    __attribute__((opencl_unroll_hint))
    SET_KNN_SET: for ( int i = 0; i < K_CONST * UNROLL_FACTOR ; ++i ) 
    {
    ...
    TRAINING_LOOP : for ( int i = 0; i < NUM_TRAINING / UNROLL_FACTOR; ++i ) 
    {
      __attribute__((opencl_unroll_hint))
      LANES : for ( int j = 0; j < UNROLL_FACTOR; j++ ) 
      {
        // Read a new instance from the training set
        WholeDigitType training_instance = training_set[j * NUM_TRAINING / UNROLL_FACTOR + i];
  ...
  results_copy = async_work_group_copy(global_results, results, NUM_TESTING, 0);
